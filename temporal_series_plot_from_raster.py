...
def get_dataframe(file_list, raster_input, parcels, num_parcels):
    ...
    for file in file_list:
        ...
        array_var = raster.read(1)
        affine = raster.transform
        #Calculating zonal statistics
        try:
            average_var = rasterstats.zonal_stats(parcels, array_var, affine = affine, stats = [stat], geojson_out = True)
        except UserWarning:
        ...
        while i < len(average_var):
            var_mean.append(average_var[i]['properties']['mean'])
            i = i + 1
